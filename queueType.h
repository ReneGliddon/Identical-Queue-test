...
/*template <class Type>
bool identicalQ (queueType<Type> Q2)
{
    bool isIdentical;
...
template<class Type>
void queueType<Type>::reverseQ(queueType< Type >&q)
{
    int stackTop;
    ...
    while(!isEmptyQueue())
    {
        localStack.push(list[queueFront]);
        stackCount++;
        count--;
...
void queueType<Type>::addQueue(const Type& newElement)
{
    if (!isFullQueue())
    {
        queueRear = (queueRear + 1) % maxQueueSize; //use the mod operator to advance queueRear
...
void queueType<Type>::deleteQueue()
{
    if (!isEmptyQueue())
    {
        ...
        //mod operator to advance queueFront because the array is circular
    }
    else
